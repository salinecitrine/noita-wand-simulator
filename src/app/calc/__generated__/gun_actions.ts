...
export const actions: Action[] = [
	...
	},
	{
		...
		action: (c: GunActionState) => {
			...
			if (( dcomps != null ) && ( dcomps.length > 0 ))  {
				;(function() {const tempArray = cloneDeep(dcomps);
	...
	},
	{
		...
		action: (c: GunActionState, recursion_level: number = 0, iteration: number = 1) => {
			...
			;(function() {const tempArray = cloneDeep(hand);
	...
	},
	{
		...
		action: (c: GunActionState) => {
			...
			;(function() {const tempArray = cloneDeep(players);
			for(let i = 0; i < tempArray.length; i++) {
				const v = tempArray[i];
	...
	},
	{
		...
		action: (c: GunActionState, recursion_level: number = 0, iteration: number = 1) => {
			...
			if ( discarded != null )  {
				;(function() {const tempArray = cloneDeep(discarded);
				for(let i = 0; i < tempArray.length; i++) {
			...
			if ( deck != null )  {
				;(function() {const tempArray = cloneDeep(deck);
				for(let i = 0; i < tempArray.length; i++) {
					const data = tempArray[i];
	...
	},
	{
		...
		action: (c: GunActionState, recursion_level: number = 0, iteration: number = 1) => {
			...
			if (( children != null ) && ( inventory != null ))  {
				...
				;(function() {const tempArray = cloneDeep(children);
				for(let i = 0; i < tempArray.length; i++) {
					...
					if ( EntityGetName( child_id ) === "inventory_quick" )  {
						...
						if ( wands != null )  {
							;(function() {const tempArray = cloneDeep(wands);
							for(let k = 0; k < tempArray.length; k++) {
								...
								if (( wand_id !== active_wand ) && EntityHasTag( wand_id, "wand" ))  {
									...
									if ( spells != null )  {
										;(function() {const tempArray = cloneDeep(spells);
			...
			if ( options.length > 0 )  {
				...
				;(function() {const tempArray = cloneDeep(actions);
				for(let i = 0; i < tempArray.length; i++) {
					const data = tempArray[i];
