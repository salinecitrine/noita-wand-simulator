...
const syntaxPatterns = [
  {pattern: /if\s+(.*?)\s+and\s+(.*?)\s+then/gm, replace: 'if ($1 && $2) then'},
  ...
  {pattern: /current_reload_time = (.*?)$/gm, replace: 'setCurrentReloadTime($1)'},
  {pattern: /mana = (.*?)$/gm, replace: 'setMana($1)'},
  {pattern: /dont_draw_actions = (.*?)$/gm, replace: 'setDontDrawActions($1)'},
  ...
  {pattern: /for (\w+)=(.+?),(.+?) do/gm, replace: 'for (let $1 = $2; $1 <= $3; $1++) {'},
  ...
  {pattern: /(data\d?|v).action\((.*?)\)/gm, replace: "call_action(ActionSource.ACTION, $1, c, $2)"},
  ...
  {pattern: /table.insert\(\s*(\w+)\s*,\s*(\w+)\s*\)/gm, replace: '$1.push($2)'},
  {pattern: /table.remove\(\s*(\w+)\s*,\s*(\w+)\s*\)/gm, replace: '$1.splice($2 - 1, 1)'},
  {pattern: /(deck|actions|hand|discarded)\[([\w.]+)]/gm, replace: '$1[$2 - 1]'},
  {pattern: /string.sub\((.*?),(.*?),(.*?)\)/gm, replace: '$1.substring($2-1,$3)'},
  {pattern: /tonumber\(/gm, replace: 'Number.parseInt('},
  {
    pattern: /(\t+)for (\w+),(\w+) in ipairs\(\s*(.+?)\s*\) do(.*?)^\1}/gms,
    replace: '$1for(let $2 = 0; $2 < $4.length; $2++) {\n$1\tconst $3 = $4[$2];\n$5$1}',
    repeat: true,
  },
  {pattern: / == /gm, replace: ' === '},
...
Deno.mkdirSync(dstFile.substring(0, dstFile.lastIndexOf('/')), {recursive: true});
